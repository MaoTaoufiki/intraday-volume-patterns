...
Each layer lives in its own ClickHouse database, the equivalent of schema separation in Postgres or Snowflake. A custom `generate_schema_name` macro overrides dbt's default behavior, which concatenates the profile schema with the custom schema and produces names like `staging_staging`. The override makes each layer write to its own database cleanly.
